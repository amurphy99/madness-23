...
derivative of a = (1-a**2)



last neuron = that neurons cost, so (a-y)**2
...
class Population:
	...
	def calculate_values(self):
		...
		#new_tanh = lambda x: (np.exp(x)-np.exp(-x))/(np.exp(x)+np.exp(-x))
		#new_tanh  = lambda x: (   exp(x)-   exp(-x))/(   exp(x)+   exp(-x))
		#new_tanh  = lambda x: torch.tanh(torch.tensor(x))
		#f_tanh = np.frompyfunc(new_tanh, 1, 1)
		...
		for i in range(1, len(self.layers)):
			...
			# do tanh to each here
			if i < len(self.layers)-1:
				# self.layers[i]["values"] = f_tanh(with_biases).astype(np.float32, casting='unsafe', copy=True)
				self.layers[i]["values"] = np.tanh(with_biases).astype(np.float32, casting='unsafe', copy=True)
			else:
				self.layers[i]["values"] = with_biases
	...
	def calc_grad_descent(self, solutions, variance):
		...
		# weight decay
		# -------------
		#weight_L1 = np.multiply(np.sum(self.layers[-1]["weights"], axis=1), 1.0)
		weight_L2 = np.multiply(np.average(np.square(self.layers[-1]["weights"]), axis=1), 0.0001)
		#weight_cost = np.add(weight_L1, weight_L2)
		...
		for i in reversed(range(1, len(self.layers)-1)):
			...
			#a1_1 = np.sum(self.layers[i+1]["w_cost"], axis=0)
			a1_1 = self.layers[i+1]["v_cost"] 
			a1_2 = np.subtract(1, np.square(self.layers[i]["values"]))

			# weight decay
			# -------------
			#weight_L1 = np.multiply(np.sum(self.layers[i]["weights"], axis=1), 1.0)
	...
	def train_and_test_stochastic(self, data, steps, batch_size):
		# given data
		training_inputs 	= data[0]
		training_solutions 	= data[1]
		training_variance   = data[2]
		...
		for j in range(steps):
			...
			for i in range(len(training_inputs)):
				...
				if count%batch_size == 0:
					average_costs.append(  sum(total_cost) / len(total_cost))
					all_accuracies.append(        accuracy / len(total_cost))
					total_cost = []
					accuracy   = 0
					self.update_weights()
			...
			# printing progress
			self.progress_bar(j, steps, step_time)
...
""".format( times[0], self.learning_rate,
	...
	def progress_bar(self, current_step, total_steps, current_time, prev_cost, prev_acc):
		# Training Progress:                                               |      steps           time      remaining  
		#		                                                           |  ----------  -------------  ------------- 
		#[######################                            ] -   44.5%    |      89/200       2m 55.1s       3m 50.7s  

		# Training Progress:                                             |   cost     acc   |      steps           time      remaining  
		#		                                                         |  ------  ------  |  ----------  -------------  ------------- 
		#[######################                            ] -   44.5%  |  18.123  50.01%  |      89/200       2m 55.1s       3m 50.7s  
