...
""".format(total_scoring_time, 
    ...
    '''
    def example_box_score_prediction(self, report_data, game_number):
        #game_number must be even
        if game_number % 2 != 0: game_number = game_number-1 

        # use the best scoring agent right now
        top = self.get_top_agent_index()

        # FIRST way around
        self.population.agents[top].set_inputs( report_data[3][game_number] )
        self.population.agents[top].calculate_value()

        outputs1 = []
        for i in range(len(self.population.agents[top].layers[-1])):
            outputs1.append(self.population.agents[top].layers[-1][i].value * max_2022[i])


        # SECOND way around
        self.population.agents[top].set_inputs( report_data[3][game_number+1] )
        self.population.agents[top].calculate_value()
        ...
        for i in range(len(self.population.agents[top].layers[-1])):
            ...
            if i < offset:
                ...
                second_value = self.population.agents[top].layers[-1][i+offset].value * max_2022[i+offset]
...
#########################################################################################################################
    ...
    def run_test(self, training_time, data, max_steps=1000):
        ...
        for i in range(max_steps):
            ...
            # --------
            scoring_start = time()
            self.population.timed_test_agents_double(training_data, training_solutions, training_variance)
        ...
        # -------------
        self.population.reset_all_scores()
        self.population.timed_test_agents_double(testing_data, testing_solutions, testing_variance)
